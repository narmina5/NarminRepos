...
def index(request):
    ...
    trendy_products = Product.objects.all().order_by('-adding_to_basket_count')[:8]
    ...
    if request.user.is_authenticated:
        if request.user.email == email:
            if not SubscribedUser.objects.filter(email=email).first():
                ...
                messages.error(request, _("Oops! You have subscribed already!"))
                return redirect('/')
        else:
            messages.error(request, _("Oops! This email doesn't belong to you!"))
            return redirect('/')
    else:
        messages.warning(request, _("Looks like you are not logged in, please log in to subscribe"))
    return redirect('/')
